...
public class CoursesToWakeUpExcel {
...
//                    e.printStackTrace();
        ...
        String choose;
        System.out.print("需要获取班级推荐课表吗？（y/n）");
    ...
    private static void getRecommendCourse(UIMSApprove uimsApprove, String user){
        ...
        System.out.println("您的班级推荐课表信息如下（请先清空模板所有数据【首行除外】，然后粘贴两条虚线间的内容至模板中，保存并导入至WakeUp课程表中）：");
...
//                    e.printStackTrace();
        ...
        FileOutputStream fileOutputStream = null;
        try {
            fileOutputStream = new FileOutputStream("班级推荐课程表_" + user + ".xlsx");
            xssfWorkbook.write(fileOutputStream);
        } catch (Exception e) {
            e.printStackTrace();
        }finally{
        ...
        System.out.println("--------------------------------------------------------------------");
        System.out.println("请先清空模板所有数据【首行除外】，然后粘贴两条虚线间的内容至模板中，保存并导入至WakeUp课程表中.");
        System.out.println("已生成 \"班级推荐课程表_" + user + ".xlsx\" ，请在程序目录查看.");
